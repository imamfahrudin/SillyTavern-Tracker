...
export function shouldGenerateTracker(mesId, type) {
	...
	debug("shouldGenerateTracker: Checking conditions", {
		mesId,
		type,
		target,
		isUser,
	...
	if (target === generationTargets.NONE || mesId < extensionSettings.generateFromMessage) {
		debug("shouldGenerateTracker: Target is NONE or below generateFromMessage threshold", { mesId, target });
		return false;
	}
...
export function shouldShowPopup(mesId, type) {
	...
	debug("shouldShowPopup: Checking conditions", {
		mesId,
		type,
		target,
		isUser,
	...
	if (target === generationTargets.NONE || mesId < extensionSettings.generateFromMessage) {
		debug("shouldShowPopup: Target is NONE or below generateFromMessage threshold", { mesId, target });
		return false;
	}
